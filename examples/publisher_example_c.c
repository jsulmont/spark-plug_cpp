...
int main(void) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (size > 0 && sparkplug_publisher_publish_data(pub, buffer, size) == 0) {
            if ((i + 1) % 5 == 0) {
                printf("✓ Published %d NDATA messages (seq: %llu)\n",
                       i + 1, (unsigned long long)sparkplug_publisher_get_seq(pub));
            }
        } else {
    ...
    printf("✓ Published 3 post-rebirth messages (seq: %llu)\n",
           (unsigned long long)sparkplug_publisher_get_seq(pub));

    // Clean disconnect
    printf("\n⏹ Disconnecting...\n");
    if (sparkplug_publisher_disconnect(pub) == 0) {
        printf("✓ Disconnected (NDEATH sent via MQTT Will)\n");
    }
